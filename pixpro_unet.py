...
device = "cuda"
encoder_weights = None  # can be None or anything else eg. "ImageNet"
dataset = "4k" # 4k or test
unlabelled = "bw+"  # for the extra bw + the portions of bitewing
...
config_unet["image_shape"] = image_shape
...
save_location = "/cluster/group/karies_2022/Simone/karies/karies-models/PixPro/unet_pixpro_weights/"

unet = U_Net_Model(config_unet, load=False)
unet_wrap = UNetModelWrapper(unet.model.encoder)

dl = PretrainingDataset(
    config_unet, DatasetPartition.train, crop_size=crop_size, dataset=dataset, unlabelled=unlabelled
).get_data_loaders()
...
for epoch in range(epochs):
    print(f"Epoch: {epoch}")
    if epoch % 200 == 0:
        model_save_name = f"unet_enc_Batch_{batch_size}_Crop_{crop_size}_After_{epoch}_epochs_{unlabelled}.pth"

        if encoder_weights is None:
            model_save_name = "FromRandom" + model_save_name
